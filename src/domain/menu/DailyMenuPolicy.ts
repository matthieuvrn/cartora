...
export class DailyMenuPolicy {
  ...
  static validateName(value: string): ValidationFailure | null {
    ...
    if (trimmed.length > DailyMenuPolicy.MAX_NAME_LENGTH)
      return { field: "name", code: "name_too_long" };
    return null;
  }

  static validateDescription(value: string): ValidationFailure | null {
    if (value.trim().length > DailyMenuPolicy.MAX_DESCRIPTION_LENGTH)
  ...
  static validatePriceCents(value: number): ValidationFailure | null {
    ...
    if (value < DailyMenuPolicy.MIN_PRICE_CENTS)
      return { field: "priceCents", code: "price_too_low" };
    if (value > DailyMenuPolicy.MAX_PRICE_CENTS)
  ...
  static validateValidUntil(validUntilISO: string, nowISO: string): ValidationFailure | null {
    ...
    if (validUntil <= now) return { field: "validUntil", code: "daily_until_in_past" };
    const maxAhead = now + DailyMenuPolicy.MAX_HORIZON_DAYS * 24 * 60 * 60 * 1000;
    if (validUntil > maxAhead) return { field: "validUntil", code: "daily_until_too_far" };
    return null;
  }

  static sanitizeName(value: string): string {
    const trimmed = value.trim();
    return trimmed.length > DailyMenuPolicy.MAX_NAME_LENGTH
      ? trimmed.slice(0, DailyMenuPolicy.MAX_NAME_LENGTH)
      : trimmed;
  }

  static sanitizeDescription(value: string): string {
    const trimmed = value.trim();
    return trimmed.length > DailyMenuPolicy.MAX_DESCRIPTION_LENGTH
      ? trimmed.slice(0, DailyMenuPolicy.MAX_DESCRIPTION_LENGTH)
      : trimmed;
  }
  ...
  static defaultExpirationISO(nowISO: string): string {
    ...
    const parts = new Intl.DateTimeFormat("en-US", {
      timeZone: DailyMenuPolicy.TIMEZONE,
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
