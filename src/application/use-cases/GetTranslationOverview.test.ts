...
describe("GetTranslationOverview", () => {
  it("reports every field missing for an enabled locale with no translation rows", async () => {
    ...
    // 3 unités traduisibles : nom de catégorie + nom d'item + description d'item.
    expect(result.coverage).toEqual([{ locale: "en", total: 3, fresh: 0, stale: 0, missing: 3 }]);
  });

  it("counts a translation fresh when its hash matches the current source", async () => {
    const menuRepo = createMockMenuRepo({ getMenuByRestaurantId: async () => menuFixture() });
    const translationRepo = createMockTranslationRepo({
  ...
  it("counts a translation stale when the source changed (hash mismatch)", async () => {
    const menuRepo = createMockMenuRepo({ getMenuByRestaurantId: async () => menuFixture() });
    const translationRepo = createMockTranslationRepo({
      listForRestaurant: async () => [
        {
    ...
    expect(result.coverage[0]).toEqual({ locale: "en", total: 3, fresh: 0, stale: 1, missing: 2 });
  });

  it("excludes fields whose source text is empty from the total", async () => {
    ...
    // category name + item name only (description source is empty).
