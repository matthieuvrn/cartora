...
export type CreateDailyDishImageUploadUrlInput = {
  restaurantId: string;
  dishId: string;
  mime: string;
};

export type CreateDailyDishImageUploadUrlOutput = SignedUploadUrl;
 ...
 * sous-chemin `{restaurantId}/daily/{dishId}.{ext}` — la RLS du bucket impose
 ...
 */
export class CreateDailyDishImageUploadUrl {
  constructor(
  ...
  async execute(
    input: CreateDailyDishImageUploadUrlInput,
  ): Promise<CreateDailyDishImageUploadUrlOutput> {
    ...
    const path = `${input.restaurantId}/daily/${input.dishId}.${ext}`;
